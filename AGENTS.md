...
- Les agents spécialisés permanents utilisent Sol `low` pour leur propre synthèse, architecture et coordination. Ils délèguent activement les analyses et travaux bornés à des sous-agents `gpt-5.6-luna` en `high` par défaut ou `xhigh` lorsque la difficulté le justifie.
