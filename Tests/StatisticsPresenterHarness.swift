...
private enum StatisticsPresenterHarness {
    ...
    static func main() {
        ...
        accumulator.ingestCoverage(channel: .screenPresence, interval: .init(start: now, duration: 200), now: now.addingTimeInterval(300), calendar: calendar)
        accumulator.recordScreenBreakEvent(.init(date: now.addingTimeInterval(200), kind: .reminded), now: now.addingTimeInterval(300))
        accumulator.recordScreenBreakEvent(.init(date: now.addingTimeInterval(220), kind: .completed), now: now.addingTimeInterval(300))
        ...
        expect(content.coverageMetrics.first?.value == "3 min", "le temps écran fiable doit apparaître en premier")
        expect(content.coverageMetrics.first(where: { $0.id == "breaks" })?.value == "1/1", "les pauses 20-20-20 doivent être présentées")
