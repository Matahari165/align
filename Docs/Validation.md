...
| Application build | `./Tools/verify.sh` | **Pending** | Release compilation, bundle assembly, packaging, and worker checks through the committed shared scheme. |
...
The two dry-run passes are useful but intentionally modest evidence. They do not prove model quality, camera performance, notifications, or the macOS UI.
...
- A real camera run, signing, notification authorization, and UI resizing have not been re-verified as part of this documentation-only change.
